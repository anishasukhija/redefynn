-- Fix Security Issues: Phase 1 Critical Fixes (Corrected)

-- 1. Since application_stats is a view, create a secure function instead
...
AS $$
  ...
  SELECT NULL::text, NULL::text, NULL::bigint, NULL::timestamp with time zone
  WHERE NOT is_admin(auth.uid())
  
  UNION ALL
  
  -- Return actual stats for admins
  SELECT 
    a.address as state,
    a.address as city,
